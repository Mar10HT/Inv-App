...
- Unit coverage of 62 percent of lines (unit suite from 102 to 600 specs): specs for the auth interceptor, the CSRF service, the xlsx contents, and the loan, sale, audit, transfer, outflow and discharge services. Specs no longer share browser storage or open a real time socket, and CI runs `npm run test:ci` with coverage thresholds that fail the build when coverage drops.
