            ...
            <table class="w-full">
              <thead>
                <tr class="bg-[var(--color-surface)]">
                  <th class="text-left px-6 py-4 text-xs font-medium text-[var(--color-on-surface-variant)] uppercase tracking-wider">{{ 'DISCHARGES.REQUESTER' | translate }}</th>
                  <th class="text-left px-6 py-4 text-xs font-medium text-[var(--color-on-surface-variant)] uppercase tracking-wider">{{ 'DISCHARGES.ITEMS' | translate }}</th>
                  <th class="text-left px-6 py-4 text-xs font-medium text-[var(--color-on-surface-variant)] uppercase tracking-wider">{{ 'DISCHARGES.WAREHOUSE' | translate }}</th>
              ...
              <tbody class="divide-y divide-[var(--color-border-subtle)]">
                @for (request of paginatedRequests(); track request.id) {
                  <tr class="hover:bg-[var(--color-surface-variant)] transition-colors cursor-pointer" (click)="viewDetail(request)">
                    <td class="px-6 py-4">
                      <div>
                        <p class="text-foreground font-medium">{{ request.requesterName }}</p>
                        <p class="text-[var(--color-on-surface-variant)] text-sm">{{ request.requesterPosition || '' }}</p>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                      <div>
                        <p class="text-foreground font-medium">{{ request.items.length }} {{ 'DISCHARGES.ITEMS_COUNT' | translate }}</p>
                        <p class="text-[var(--color-on-surface-variant)] text-sm truncate max-w-xs">{{ getItemsPreview(request) }}</p>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                  ...
                  <tr>
                    <td colspan="6" class="px-6 py-16 text-center">
                      ...
                      <h3 class="text-lg font-semibold text-[var(--color-on-surface-variant)] mb-2">{{ 'DISCHARGES.NO_REQUESTS' | translate }}</h3>
                      <p class="text-[var(--color-on-surface-muted)]">{{ 'DISCHARGES.NO_REQUESTS_DESC' | translate }}</p>
                    </td>
                  </tr>
                }
              </tbody>
            ...
            @for (request of paginatedRequests(); track request.id) {
              <div class="p-4" (click)="viewDetail(request)">
                <div class="flex justify-between items-start mb-3">
                  <div>
                    <p class="text-foreground font-medium">{{ request.requesterName }}</p>
                    <p class="text-[var(--color-on-surface-variant)] text-sm">{{ request.items.length }} {{ 'DISCHARGES.ITEMS_COUNT' | translate }}</p>
                  </div>
                  <span [class]="getStatusClass(request.status)" class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium">
                    {{ getStatusLabel(request.status) }}
                  </span>
                ...
                <div class="grid grid-cols-2 gap-3 text-sm mb-3">
                  <div>
                    <p class="text-[var(--color-on-surface-variant)]">{{ 'DISCHARGES.WAREHOUSE' | translate }}</p>
      ...
      <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" (click)="closeShareDialog()">
        <div class="rounded-xl w-full max-w-md border border-theme" [style.background-color]="'var(--color-surface-variant)'" (click)="$event.stopPropagation()">
          ...
          <div class="p-6 flex flex-col items-center gap-5">
            <p class="text-sm text-[var(--color-on-surface-variant)] text-center">{{ 'DISCHARGES.SHARE_FORM.DESCRIPTION' | translate }}</p>
            ...
            @if (shareLoading()) {
              ...
              <div class="w-full rounded-lg px-4 py-3 flex items-center gap-2 border border-theme" [style.background-color]="'var(--color-surface-elevated)'">
                <lucide-icon name="Link" class="!w-4 !h-4 !text-[var(--color-on-surface-variant)] shrink-0"></lucide-icon>
              ...
              <div class="flex gap-3 w-full">
                ...
                <button
                  (click)="downloadQr()"
                  class="flex items-center justify-center gap-2 px-4 py-2.5 bg-transparent border border-theme text-[var(--color-on-surface-variant)] rounded-lg transition-colors">
